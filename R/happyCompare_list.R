...
#'   \itemize{ \item{\code{samplesheet}: the original samplesheet, stored as a
#'   \code{data.frame}}. \item{\code{happy_results}: a \code{happy_results_list}
...
read_samplesheet = function(samplesheet_path, lazy = TRUE) {
  ...
  # load happy results
  happy_results = lapply(samplesheet$.Id, function(id) {
    message(sprintf("Processing %s", id))
  ...
  names(happy_results) = samplesheet$.Id
