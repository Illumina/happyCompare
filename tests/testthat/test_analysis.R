...
test_that("estimate_hdi works", {
  ...
  e1 = estimate_hdi(happy_extended = d, successes_col = successes_col, totals_col = totals_col, 
                   group_cols = group_cols, aggregate_only = TRUE, sample_size = 1e3)
  e2 = estimate_hdi(happy_extended = d, successes_col = successes_col, totals_col = totals_col, 
                    group_cols = group_cols, aggregate_only = FALSE, sample_size = 1e3)
  
  # happy_extended contains subsets with truth.total == 0, check that they've been excluded from output
  expect_equal(dim(e1 %>% filter(replicate_id != ".aggregate"))[1], dim(d)[1] - dim(d %>% filter(TRUTH.TOTAL == 0))[1])
  expect_equal(dim(e2 %>% filter(replicate_id != ".aggregate"))[1], dim(d)[1] - dim(d %>% filter(TRUTH.TOTAL == 0))[1])
  
  # check output cols
  required_cols = c(group_cols, "replicate_id", "successes", "totals", "mu", "sigma", 
                    "alpha0", "beta0", "alpha1", "beta1", "lower", "observed_p", "estimated_p", "upper", "hdi_range")
